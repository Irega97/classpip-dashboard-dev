...
import { Pregunta, Profesor, User } from 'src/app/clases';
...
export class LoginComponent implements OnInit {
  ...
  Autentificar() {
    ...
    let user: User;
    user = {"username":this.nombre, "email":"", "password":this.pass};

    this.peticionesAPI.login(user).subscribe((res) => {
      console.log(res);
      if(res !== undefined) {
        console.log ('autentificado correctamente');
        const token: string = res.id;
        const userId: number = res.userId;
        console.log('token: ', token, ' | userId: ', userId);
        sessionStorage.setItem('ACCESS_TOKEN', token);
        this.peticionesAPI.DameProfesor(userId)
        .subscribe(
          (prof) => {
            console.log ('profesor obtenido: ', prof);
            if (prof[0] !== undefined) {
              this.profesor = prof[0]; // Si es diferente de null, el profesor existe y lo meto dentro de profesor
